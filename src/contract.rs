use cosmwasm_std::{
    ...
    Env, MessageInfo, BankMsg, Response, StdResult, Uint128, WasmMsg, Coin, Order,QueryRequest,WasmQuery
};
...
use crate::state::{
    State,CONFIG,TOKENINFO,OWNEDTOKEN, TokenInfo, CLAIMFLAG, Image
};
...
pub fn instantiate(
    ...
    let state = State {
        ...
        monthly_reward:msg.monthly_reward,
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::SetMonthlyReward { amount } => execute_set_reward(deps, env, info, amount),
...
fn execute_claim_reward(
    ...
    let state = CONFIG.load(deps.storage)?;
    let mut claim_number = Uint128::new(0);

    for token_id in token_ids{
        let is_claim = CLAIMFLAG.may_load(deps.storage, &token_id)?;
        if is_claim != None{
            continue;
        }
        else{
            let token_info :AllNftInfoResponse<Image> = deps.querier.query(&QueryRequest::Wasm(WasmQuery::Smart {
                contract_addr: state.nft_address.clone(),
                msg: to_binary(&Cw721QueryMsg::AllNftInfo { 
                    token_id: token_id.clone(), 
                    include_expired:None })?,
            }))?;
            if token_info.access.owner != info.sender.to_string() {
                continue;
            }
            else{
                let flag =  true;
                CLAIMFLAG.save(deps.storage, &token_id.clone(), &flag)?;
                claim_number = claim_number+Uint128::new(1);
            }
        }
    }
    
    if claim_number == Uint128::new(0){
        return Err(ContractError::NoNftClaim {  })
    }
   ...
   Ok(Response::new()
        .add_message(CosmosMsg::Wasm(WasmMsg::Execute {
             ...
             msg: to_binary(&Cw20ExecuteMsg::Transfer {
                 recipient: info.sender.to_string(), 
                 amount: state.claim_reward * claim_number
                })? , 
            funds: vec![] }))
   ...
   if token_group.len() == 0 {
    ...
    for token_id in token_group{
            ...
            {       TOKENINFO.update(deps.storage, &token_id,
                |token_info|->StdResult<_>{
                    let mut token_info = token_info.unwrap();
                    token_info.reward = token_info.reward + state.monthly_reward/state.total_staked;
                    Ok(token_info)
            }
            )?; }
    }
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
          ...
          QueryMsg::GetClaimAmount { id, address } => to_binary(&query_claim_amount(deps,id,address)?)
  }
}
...
pub fn query_claim_amount(deps:Deps,id:Vec<String>,address: String)->StdResult<Uint128>{
    let state = CONFIG.load(deps.storage)?;
    let mut claim_number = Uint128::new(0);
    ...
    for token_id in id{
        ...
        if is_claim != None{
            ...
            if token_info.access.owner != address {
                continue;
            }
                claim_number = claim_number+Uint128::new(1);
    ...
    Ok(state.claim_reward*claim_number)
}
...
mod tests {
    ...
    fn testing() {
        ...
        let instantiate_msg = InstantiateMsg {
            ...
            monthly_reward:Uint128::new(10000),
